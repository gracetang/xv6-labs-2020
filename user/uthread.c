...
//Save registers for thread context switches
struct context
{
  uint64 ra;
  uint64 sp;
...
thread_create(void (*func)())
{
  ...
  t->state = RUNNABLE;
  //thread_create will return to ra, and then running the thread
  t->ctx.ra = (uint64) func; // return address
  // set thread's stack pointer to it's own stack, 
  // notice that stack grows from high address to low address
